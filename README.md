...
- таблицы `tags` и `tag_assignments(tag_id, entity_type, entity_id)`; ключ типа `entity_type` —
...
Внешнего ключа `tag_assignments → запись` нет и быть не может (таблицы записей — в разных
